...
void Console::processInput()
{
	...
	if (currentInput == "TogClip")
		...
		writeToSave(currentInput.substr(5)); // Save everything after "Save "

	else if (currentInput == "Decrypt")
		decrpytSave();

	else if (currentInput.substr(0, 5) == "Read ")
		readFromFile(currentInput.substr(5)); // Read everything after "Read "
		...
		playSong(currentInput.substr(5)); // Process everything after "Play "
...
{
	// Writes whatever is in input to the save file.
...
void Console::decrpytSave()
{
	...
	console_log.push_back(Jesus.readSave());
	console_color.push_back(VALID_COLOR);
}
